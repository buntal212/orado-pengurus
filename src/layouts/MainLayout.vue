      ...
      <q-toolbar class="sso-toolbar">
        ...
        <q-btn flat round dense icon="notifications_none" aria-label="Notifikasi">
          <q-badge
            ...
            :label="badgeLabel"
          />
          <q-menu class="notification-menu" @show="notifikasi.getData">
            <q-card flat class="notification-card">
              <div class="notification-heading">
                <div>
                  <strong>Notifikasi</strong>
                  <span>{{
                    notifikasi.items.length ? 'Notifikasi terbaru ORADO' : 'Belum ada notifikasi'
                  }}</span>
                </div>
                <q-btn
                  v-if="notifikasi.jumlahBelumDibaca"
                ...
                />
                <q-icon v-else name="notifications" color="primary" size="22px" />
              </div>
              <q-separator />
                ...
                >
                  ...
                  <q-item-section>
                    ...
                    <q-item-label caption class="notification-time">{{
                      waktuNotifikasi(item.dibuat_pada)
                    }}</q-item-label>
                  </q-item-section>
                  <q-item-section v-if="!item.dibaca" side top
