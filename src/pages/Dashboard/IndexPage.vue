    ...
    <main class="sso-content">
      ...
      <section class="dashboard-panel">
        ...
        <article class="balance-card">
          <div>
            <span>Saldo organisasi</span><strong>Rp 67.850.000</strong
            ><small><q-icon name="trending_up" /> Stabil bulan ini</small>
          </div>
...
const activities = [
  ...
  {
    title: 'Laporan keuangan diterima',
...
.welcome-card {
  ...
  border-radius: 14px;
  background: #fff;
  box-shadow: 0 10px 26px rgba(12, 40, 79, 0.16);
}
.welcome-avatar {
  display: grid;
  width: 64px;
...
.menu-panel {
  ...
  border-radius: 15px;
  background: #fff;
  box-shadow: 0 9px 22px rgba(27, 57, 96, 0.1);
}
.menu-card {
  min-height: 102px;
  padding: 12px 5px;
  ...
  border-radius: 11px;
  background: #fff;
  box-shadow: 0 2px 5px rgba(20, 57, 99, 0.04);
  cursor: pointer;
}
.menu-card:active {
  background: #edf5ff;
...
.dashboard-panel {
  ...
  box-shadow: 0 10px 28px rgba(27, 57, 96, 0.1);
}
.stat-grid {
  display: grid;
...
.stat-card:hover {
  box-shadow: 0 7px 16px rgba(27, 57, 96, 0.12);
  transform: translateY(-1px);
}
.stat-icon {
  display: grid;
...
.balance-card {
  ...
  border-radius: 14px;
  color: #fff;
  background: linear-gradient(135deg, #062d61, #0b66c6);
  box-shadow: 0 8px 18px rgba(8, 78, 161, 0.22);
}
.balance-card::before {
  position: absolute;
  right: -25px;
...
.account-card {
  margin-top: 12px;
  box-shadow: 0 3px 10px rgba(26, 54, 91, 0.05);
}
.account-copy {
  min-width: 0;
  flex: 1;
